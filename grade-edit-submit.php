...
while( list( $field, $value ) = each( $gradeArray )) {
	...
	if ( (strpos($field, 'criteria') !== false) && is_numeric($value) ) {
	 	...
	 	$gradedRows = mysql_query("SELECT * FROM rubric_grade_answers WHERE answer_grade_id = '$gradeID' AND answer_criteria_id = '$gradeCriteriaID' AND is_comment= '0' AND is_textbox = '0'");
	...
	// updates comments from database
			...
			$gradeCriteriaID = substr($field,8);
		 	...
		 	$gradedRows = mysql_query("SELECT * FROM rubric_grade_answers WHERE answer_grade_id = '$gradeID' AND answer_criteria_id = '$gradeCriteriaID' AND is_comment= '1' AND is_textbox = '0'");
	 		$gradedRowsCount = mysql_num_rows($gradedRows);
		...
		// updates textbox content from database
				...
				$gradeCriteriaID = substr($field,8);
		 		...
		 		$gradedRows = mysql_query("SELECT * FROM rubric_grade_answers WHERE answer_grade_id = '$gradeID' AND answer_criteria_id = '$gradeCriteriaID' AND is_comment= '0' AND is_textbox = '1'");
	 			$gradedRowsCount = mysql_num_rows($gradedRows);
